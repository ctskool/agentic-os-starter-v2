...
function vault(t){
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'headless-adapter-'));
 // Async removal retries Windows EBUSY while a terminated child's cwd handle is released.
 t.after(()=>fs.promises.rm(root,{recursive:true,force:true,maxRetries:10,retryDelay:200}));
 return root;
}
