...
for(const surfaceKind of ['native','web'])test(`Mac shortcut uses the ${surfaceKind==='native'?'background native':'visible HUD'} microphone once and submits after speech and silence`,async t=>{
 const f=audioFixture(t,{surfaceKind,hidden:surfaceKind==='native',pendingMic:true}),recorders=captureRecorder(t);await flush();
