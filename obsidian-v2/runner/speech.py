...
def create_app(assets, engines=None, capture_factory=CaptureController, hotkey_factory=HotkeyListener):
    ...
    capture = capture_factory(transcribe_pcm, emit,
    ...
    def trigger(source='hotkey'):
        return capture.trigger(source) if clients else False
    ...
    def health():
        return {'ok': True, 'engine': 'kokoro', 'stt': {'ok': True},
            ...
            'hotkey': {'enabled': hotkey.enabled, 'combo': combo,
                'ok': bool(capture.available and hotkey.ok), 'error': capture.error or hotkey.error}}
