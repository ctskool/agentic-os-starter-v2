...
export default function GalaxyCore({ mode = "idle", paused = false, variant = "tight", compact = false, getLevel }: { mode?: CoreMode; paused?: boolean; variant?: GalaxyVariant; compact?: boolean; getLevel?:()=>number|null }) {
  ...
  useEffect(() => {
    ...
    try { renderer = new THREE.WebGLRenderer({ canvas: element.ownerDocument.createElement('canvas'), alpha: true, antialias: false }); }
    catch { setUnavailable(true); return; }
    renderer.setPixelRatio(Math.min(view.devicePixelRatio, 1.6));
    ...
    // The floating bubble keeps the same four-arm geometry with fewer, finer stars.
    const count = compact ? 4200 : view.innerWidth < 700 ? 19000 : 37000;
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: { uTime: { value: 0 }, uEnergy: { value: 0 }, uListen: { value: 0 }, uRatio: { value: renderer.getPixelRatio() }, uBloom: { value: 0 }, uPointScale: { value: compact ? 0.26 : 1 } },
      vertexShader: `
        attribute float aSeed; attribute float aSize;
        uniform float uTime; uniform float uEnergy; uniform float uListen; uniform float uRatio; uniform float uBloom; uniform float uPointScale;
        ...
        void main() {
          ...
          gl_PointSize = clamp(aSize * uPointScale * mix(1.0, 3.2, uBloom) * uRatio * (9.0 / -mv.z), 1.0, 80.0);
    ...
    const textureCanvas = element.ownerDocument.createElement("canvas");
    ...
    const resize = () => {
      ...
      camera.position.z = compact ? 9.6 : w < 700 ? 12.5 : 10;
      camera.updateProjectionMatrix();
    };
    const observer = new view.ResizeObserver(resize); observer.observe(element); resize();
    const frames=visibleFrames(element,error=>{recovery.fail();console.warn('[Agentic OS] Galaxy rendering paused.',error)});
    let frame = 0, time = 0, breathTime = 0, previous = performance.now(), lastDraw = 0;
    const animate = (now: number) => {
      frame = frames.request(animate);
      const dt = Math.min((now - previous) / 1000, 0.05); previous = now;
      if (element.ownerDocument.hidden || !element.getClientRects().length || now - lastDraw < 30) return;
      ...
      const target = current === "speaking" ? (feel.current.getLevel?.() ?? (0.65 + Math.sin(time * 7) * 0.3)) : current === "working" ? 0.45 : current === "listening" ? 0.25 : 0;
      ...
      disk.rotation.z = -0.3 + time * (compact ? 0.065 * 1.5 : 0.045);
      ...
      nucleus.scale.setScalar(1.60 + breath * 0.14 + material.uniforms.uEnergy!.value * 0.25);
      nucleusMaterial.opacity = current === "error" ? 0.45 : 0.82 + breath * 0.12;
      aura.scale.setScalar(3.3 + breath * 0.3);
      auraMaterial.opacity = current === "error" ? 0.04 : 0.09 + breath * 0.04;
      renderer.render(scene, camera);
    };
    const recovery=watchWebGLContext(renderer.domElement,frames,setUnavailable,()=>{
      dirty=true;drew=false;lastDraw=0;previous=performance.now();resize();frames.request(animate);
    });
    frame = frames.request(animate);
    return () => {
      recovery.dispose();frames.dispose(); observer.disconnect();
      geometry.dispose(); material.dispose(); texture.dispose(); nucleusMaterial.dispose();
      bloomGeometry.dispose(); bloomMaterial.dispose();
      auraMaterial.dispose(); renderer.dispose(); renderer.forceContextLoss(); renderer.domElement.remove();
    };
  }, [variant, compact]);
