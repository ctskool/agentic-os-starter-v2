...
export default function GraphCore({
  ...
  useEffect(() => {
    ...
    let simT = 0;
    let lastDeg = -1;
    const frames=visibleFrames(mount);
    let raf = 0;
    const tick = () => {
      if (document.hidden || ((pausedRef.current || window.matchMedia("(prefers-reduced-motion: reduce)").matches) && raf)) { raf = frames.request(tick); return; }
