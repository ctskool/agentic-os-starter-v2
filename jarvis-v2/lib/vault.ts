...
let VAULT_ROOT = path.resolve(process.cwd(), "../obsidian-v2/test-vault");
export function setVaultRoot(root:string){VAULT_ROOT=fs.realpathSync(root);}
...
export function readVaultState(includeRuntime=true): VaultState {
  return {
    ...
    metrics: readMetrics(),
    runner: includeRuntime?readRunnerStatus():null as any,
...
// Shared across requests/provider modes. External vault edits appear within ten seconds.
const dashboardCache=createResourceCache(10000);
export function invalidateVaultSnapshot(root = VAULT_ROOT){dashboardCache.invalidate(root)}
export function readCachedVaultState():Promise<VaultState>{return dashboardCache.read(VAULT_ROOT,()=>readVaultState(false))}
